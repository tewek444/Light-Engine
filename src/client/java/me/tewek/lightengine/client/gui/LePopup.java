...
public class LePopup {
    ...
    public void renderBackground(GuiGraphicsExtractor graphics, int screenWidth, int screenHeight) {
        if (this.dim) {
            graphics.fill(0, 0, screenWidth, screenHeight, LeTheme.DIM);
        }
        int px = this.panelX(screenWidth);
        int py = this.panelY(screenHeight);
        LeTheme.frame(graphics, px, py,
    ...
    public void renderWidgets(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
