    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = first; i <= last; i++) {
            ...
            if (isHeader) {
                ...
                graphics.drawString(font, this.trimToWidth(font, entry.label, this.width - 16),
            ...
            if (entry.icon != null && !entry.icon.isEmpty()) {
                graphics.renderItem(entry.icon, this.x + 4, rowY + (this.rowHeight - 16) / 2);
            }
            ...
            graphics.drawString(font, this.trimToWidth(font, entry.label, Math.max(8, labelW)),
                    labelX, rowY + 3, i == this.selected ? LeTheme.ACCENT : LeTheme.TEXT);
            if (!entry.sub.isEmpty()) {
                graphics.drawString(font, entry.sub,
