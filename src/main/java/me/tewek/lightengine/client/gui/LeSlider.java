...
public class LeSlider implements LeWidget {
    ...
    public LeSlider(int x, int y, int width, int height, Component label,
        ...
        this.value = Math.clamp(value, min, this.max);
    ...
    public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (labeled) {
            ...
            graphics.text(Minecraft.getInstance().font, text, this.x, this.y, LeTheme.TEXT);
        }
        ...
        if (this.markedValue != null) {
            double mfrac = (double) (Math.clamp(this.markedValue, this.min, this.max) - this.min)
